...
def compareCSV(file1, file2):
	...
	df_2 = pd.read_csv(os.path.dirname(os.path.realpath(__file__)) + '/inc/' + file2, names=['title','id','price','currency','city','county','district','lat','lon','date','brand','series','model','fuel','year','gear','km','frame','enginevolume','horsepower','drivetype','color','warranty','plate','saler','status','exchange'])
	for x in df_2.id:
		id_2.append(int(x))
...
def compareCSV2(file1, file2):
	...
	df_1 = pd.read_csv(os.path.dirname(os.path.realpath(__file__)) + '/inc/' + file1, names=['title','id','price','currency','city','county','district','lat','lon','date','brand','series','model','fuel','year','gear','km','frame','enginevolume','horsepower','drivetype','color','warranty','plate','saler','status','exchange'])
	for x in df_1.id:
		id_1.append(int(x))
...
if __name__ == '__main__':
	clearDuplicates('vehicle_data.csv')
	clearDuplicates('id_url.csv')
	compareCSV('id_url.csv','vehicle_data.csv')
	removeVal()
	compareCSV2('vehicle_data.csv','id_url.csv')
